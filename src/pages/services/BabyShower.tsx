...
const BabyShower = () => {
  const svc = useServiceContent('baby-shower');
  const d = svc?.data;

  return (
    <>
      <SEO
        title={d?.seoTitle || "Baby Shower Sarajevo | 300 KM | The Stage"}
        description={d?.seoDescription || "Organizacija baby shower proslava u Sarajevu. Fotogeničan prostor, snack bar i dekoracija. Cijena 300 KM. Rezervišite termin!"}
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center animate-fadeIn">
            ...
            <h1 className="text-5xl md:text-6xl font-serif font-bold text-dark-grey mb-4">
              {d?.heroTitle || "Baby Shower Sarajevo"}
            </h1>
            <p className="text-xl text-soft-grey max-w-2xl mx-auto px-4">
              {d?.heroTagline || "Savršeno okruženje da buduću mamu iznenadite poklonima"}
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="bg-white rounded-lg shadow-elegant p-6 sm:p-8 mb-12 border border-gold/20">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
            <div className="flex flex-col items-center">
              ...
              <p className="text-2xl font-serif text-gold font-bold">{d?.stat1Value || "300 KM"}</p>
              <p className="text-soft-grey text-sm">{d?.stat1Label || "Kompletna proslava"}</p>
            </div>
            <div className="flex flex-col items-center">
              <Camera className="w-8 h-8 text-gold mb-2" />
              <p className="text-2xl font-serif text-dark-grey font-bold">{d?.stat2Value || "Fotogenično"}</p>
              <p className="text-soft-grey text-sm">{d?.stat2Label || "Savršeno za uspomene"}</p>
            </div>
            <div className="flex flex-col items-center">
              <Heart className="w-8 h-8 text-gold mb-2" />
              <p className="text-2xl font-serif text-dark-grey font-bold">{d?.stat3Value || "Snack Bar"}</p>
              <p className="text-soft-grey text-sm">{d?.stat3Label || "Neograničeni sokovi"}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-soft p-8 md:p-12 mb-12">
          ...
          <div className="mb-8">
            <p className="text-soft-grey mb-6 leading-relaxed text-lg">
              {d?.mainDescription || "Tražite savršen prostor za baby shower u Sarajevu? The Stage nudi savršeno okruženje za proslavu baby shower-a. U fotogeničnom i prikladno uređenom prostoru, stvorite nezaboravne trenutke sa budućom mamom i najbližima."}
            </p>
