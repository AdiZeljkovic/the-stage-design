...
const ItalianNight = () => {
  const svc = useServiceContent('italian-night');
  const d = svc?.data;

  return (
    <>
      <SEO
        title={d?.seoTitle || "Italian Night Sarajevo | Pasta & Spritz | The Stage"}
        description={d?.seoDescription || "Italian Night u Sarajevu - naučite praviti pastu i talijanska pića. Savršen date night ili izlazak s prijateljima!"}
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center animate-fadeIn">
            ...
            <h1 className="text-5xl md:text-6xl font-serif font-bold text-dark-grey mb-4">
              {d?.heroTitle || "Italian Night Sarajevo"}
            </h1>
            <p className="text-xl text-soft-grey max-w-2xl mx-auto px-4">
              {d?.heroTagline || "Pasta & Spritz - Iskustvo talijanskog glamura"}
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="bg-white rounded-lg shadow-elegant p-6 sm:p-8 mb-12 border border-gold/20">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
            <div className="flex flex-col items-center">
              ...
              <p className="text-soft-grey text-sm">{d?.stat1Label || "Cijena"}</p>
            </div>
            <div className="flex flex-col items-center">
              <Users className="w-8 h-8 text-gold mb-2" />
              <p className="text-2xl font-serif text-dark-grey font-bold">{d?.stat2Value || "18+"}</p>
              <p className="text-soft-grey text-sm">{d?.stat2Label || "Samo punoljetni"}</p>
            </div>
            <div className="flex flex-col items-center">
              <Utensils className="w-8 h-8 text-gold mb-2" />
              <p className="text-2xl font-serif text-dark-grey font-bold">{d?.stat3Value || "Pasta & Spritz"}</p>
              <p className="text-soft-grey text-sm">{d?.stat3Label || "Autentično iskustvo"}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-soft p-8 md:p-12 mb-12">
          ...
          <div className="mb-8">
            <p className="text-soft-grey mb-6 leading-relaxed text-lg">
              {d?.mainDescription || "Tražite jedinstveno iskustvo u Sarajevu? Italian Night je savršen 'date night' ili izlazak s prijateljima! Doživite autentično italijansko iskustvo dok pravite pastu od nule – od brašna, jaja i soli."}
            </p>
            ...
            {(d?.note || "x") !== "x" && (
              <div className="p-6 bg-gold/10 rounded-lg border border-gold/20 mb-6">
                <p className="text-dark-grey font-semibold mb-2">Važna Napomena:</p>
                <p className="text-soft-grey">{d?.note}</p>
              </div>
            )}
            {!d?.note && (
              <div className="p-6 bg-gold/10 rounded-lg border border-gold/20 mb-6">
                <p className="text-dark-grey font-semibold mb-2">Važna Napomena:</p>
                <p className="text-soft-grey">Potrebno je rezervirati mjesto unaprijed. Italian Night se organizuje na zahtjev za grupe, kontaktirajte nas za dostupne termine.</p>
              </div>
            )}
