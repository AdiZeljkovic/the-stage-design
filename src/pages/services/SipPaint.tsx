...
const SipPaint = () => {
  const svc = useServiceContent('sip-paint');
  const d = svc?.data;

  return (
    <>
      <SEO
        title={d?.seoTitle || "Sip and Paint Sarajevo | The Stage"}
        description={d?.seoDescription || "Sip & Paint večeri u Sarajevu - slikanje uz koktele. Svakog drugog četvrtka. Idealan izlazak s prijateljicama!"}
      ...
      <section className="relative h-[60vh] bg-gradient-to-br from-gold/20 to-cream overflow-hidden">
        <img
          ...
          alt="Sip and Paint večer u Sarajevu - The Stage"
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center animate-fadeIn">
            ...
            <h1 className="text-5xl md:text-6xl font-serif font-bold text-dark-grey mb-4">
              {d?.heroTitle || "Sip and Paint Sarajevo"}
            </h1>
            <p className="text-xl text-soft-grey max-w-2xl mx-auto px-4">
              {d?.heroTagline || "Opustite se uz kist, platno i koktel"}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="bg-white rounded-lg shadow-elegant p-6 sm:p-8 mb-12 border border-gold/20">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
            ...
            <div className="flex flex-col items-center">
              ...
              <p className="text-2xl font-serif text-dark-grey font-bold">{d?.stat3Value || "Drugi Četvrtak"}</p>
              <p className="text-soft-grey text-sm">{d?.stat3Label || "20:00-22:00h"}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-soft p-8 md:p-12 mb-12">
          ...
          <div className="mb-8">
            <p className="text-soft-grey mb-6 leading-relaxed text-lg">
              {d?.mainDescription || "Tražite kreativno iskustvo u Sarajevu? Izvedi prijateljicu, mamu ili sestru na nezaboravno Sip and Paint iskustvo! Svakog drugog četvrtka u mjesecu (20:00 - 22:00h) pijuckamo koktele, slušamo muziku i opuštamo se uz kist i platno."}
            </p>
        ...
        <div className="text-center">
          <Button asChild size="lg" className="bg-gold hover:bg-gold/90 text-warm-white">
            <Link to="/kontakt">{d?.ctaText || "Rezervišite mjesto"}</Link>
          </Button>
        </div>
      </section>
      </div>
